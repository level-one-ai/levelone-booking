...
export interface BookingEmailParams {
  clientName: string; clientEmail: string; clientPhone: string; company: string;
  location: string; website?: string; callType: "phone" | "video";
...
<p style="color:rgba(255,255,255,0.85);margin:4px 0 0;font-size:13px">Level One — ${isVideo ? "Video Call" : "Phone Call"}</p></div>
...
${isVideo && p.zoomJoinUrl ? `<a href="${p.zoomJoinUrl}" style="display:block;background:#2D8CFF;color:#fff;text-decoration:none;padding:12px;border-radius:8px;text-align:center;font-weight:600;margin-top:16px">Join Zoom Meeting</a>` : ""}
