...
export interface BookingEmailParams {
  ...
  meetLink?: string; // Google Meet join URL
}
...
<p style="color:rgba(255,255,255,0.85);margin:4px 0 0;font-size:13px">Level One — ${isVideo ? "Video Call (Google Meet)" : "Phone Call"}</p></div>
<table style="width:100%;border-collapse:collapse">
${[["Date",p.date],["Time",`${p.time} (London)`],["Duration",`${p.durationMinutes} minutes`],["Type",isVideo?"Video call (Google Meet)":"Phone call"]].map(([l,v])=>`<tr><td style="padding:8px;color:#999;font-size:13px">${l}</td><td style="padding:8px;color:#fff;font-size:13px;text-align:right">${v}</td></tr>`).join("")}
</table>
${isVideo && p.meetLink ? `
<div style="margin-top:16px;background:#1e3a2e;border:1px solid #2d5c44;border-radius:8px;padding:16px;text-align:center">
  <p style="color:#6ee7b7;font-size:12px;margin:0 0 10px;font-weight:600">GOOGLE MEET LINK</p>
  <a href="${p.meetLink}" style="display:inline-block;background:#1a73e8;color:#fff;text-decoration:none;padding:12px 24px;border-radius:8px;font-weight:600;font-size:14px">Join Google Meet</a>
  <p style="color:#888;font-size:11px;margin:10px 0 0;word-break:break-all">${p.meetLink}</p>
</div>` : ""}
...
<table style="width:100%;border-collapse:collapse">
${([ ["Client",p.clientName],["Company",p.company],["Email",p.clientEmail],["Phone",p.clientPhone],["Location",p.location],p.website ? ["Website",p.website] : null,["Date",p.date],["Time",p.time],["Duration",`${p.durationMinutes} min`],["Type",p.callType==="video"?"Video call (Google Meet)":"Phone call"] ] as (string[] | null)[]).filter((x): x is string[] => x !== null).map(([l,v])=>`<tr><td style="padding:7px;color:#888;font-size:13px">${l}</td><td style="padding:7px;color:#fff;font-size:13px">${v}</td></tr>`).join("")}
</table>
${p.meetLink ? `
<div style="margin-top:16px;background:#1e3a2e;border:1px solid #2d5c44;border-radius:8px;padding:14px;text-align:center">
  <p style="color:#6ee7b7;font-size:12px;margin:0 0 8px;font-weight:600">GOOGLE MEET</p>
  <a href="${p.meetLink}" style="display:inline-block;background:#1a73e8;color:#fff;text-decoration:none;padding:10px 20px;border-radius:6px;font-weight:600;font-size:13px">Join meeting</a>
</div>` : ""}
</div>`,
  });
}
